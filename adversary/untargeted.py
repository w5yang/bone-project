...
class TargetBalance(Adversary):
    def __init__(self, args, **kwargs):
        ...
        self.penalty_matrix = torch.zeros(
            (self.sampler.num_classes, self.sampler.num_classes)
        )
        self.temp_penalty = torch.zeros(
            (self.sampler.num_classes, self.sampler.num_classes)
        )
        self.statistic = torch.zeros(
            (self.sampler.num_classes, self.sampler.num_classes), dtype=int
        )
        self.max_iter = 200
        self.overshoot = 0.02
    ...
    def choose(self, budget: int):
        ...
        self.direction_tensor = torch.cat(directions, dim=0)
