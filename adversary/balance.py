...
class BalanceAdversary(Adversary):
    def __init__(self, args, **kwargs):
        ...
        if kwargs['test']:
            ...
            self.sampler.dataset = Subset(self.sampler.dataset, [i for i in perm[:20000]])
            self.verbose = True
    ...
    def get_perturbation(self, item: Union[int, torch.Tensor]) -> Tuple[float, Tuple[int, int]]:
        ...
        penalty = torch.zeros([self.num_classes], device=self.device, dtype=torch.float32)
    ...
    def deepfool_synthetic(self, item: Union[int, torch.Tensor]) -> Tuple[torch.Tensor, float, Tuple[int, int]]:
        self.model.model.eval()
        softmax = torch.nn.Softmax(1)
        if isinstance(item, int):
            sample: torch.Tensor = self.sampler.dataset[item][0].to(self.device)
    ...
    def choose(self, method: str, budget: int):
        if method == 'random':
            ...
            # selected = []
            # for t, B in enumerate(range(0, len(self.sampler.dataset), 100)):
            #     x_t = torch.stack([self.sampler.dataset[i][0] for i in range(B, min(B + 100, len(self.sampler.dataset)))]).to(self.device)
            #     y_t = self.blackbox(x_t)
            #     for i in range(x_t.size(0)):
            #         selected.append((x_t[i].cpu(), y_t[i].cpu()))
            # self.model.train(selected)
        elif method == 'balance':
...
def main():
    ...
    parser.add_argument('--method', metavar='M', type=str, help='Determine sample/synthetic method.',
                        default='balance', choices=['balance', 'deepfool', 'random', 'balance_pert', 'deepfool_pert'])
    ...
    for i in range(2):
        ...
        np.save(os.path.join(adversary.model.model_dir, 'selected_.npy'), adversary.sampler.indices)
