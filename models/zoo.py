...
def get_net(
    modelname: str, inputsize: int, pretrained: str = None, num_classes=1000, **kwargs
):
    if inputsize == 32:
        modeltype = "tiny"
        if pretrained and pretrained is not None:
            return get_pretrainednet(
                modelname, modeltype, pretrained, num_classes, **kwargs
            )
        else:
            try:
...
def get_pretrainednet(
    modelname, modeltype, pretrained="imagenet", num_classes=1000, **kwargs
):
    if pretrained == "imagenet":
        ...
        raise ValueError(
            "Currently only supported for imagenet or existing pretrained models"
        )
...
def get_imagenet_pretrainednet(modelname, num_classes=1000, **kwargs):
    ...
    assert (
        modelname in valid_models
    ), "Model not recognized, Supported models = {}".format(valid_models)
...
def copy_weights_(src_state_dict, dst_state_dict):
    ...
    for i, (src_param_name, src_param) in enumerate(src_state_dict.items()):
        if src_param_name in dst_state_dict:
            ...
            n_skipped += 1
    print(
        "=> # Success param blocks loaded = {}/{}, "
        "# Skipped = {}, # Shape-mismatch = {}".format(
            n_success, n_params, n_skipped, n_shape_mismatch
        )
    )
