...
class DeepFool(Attack):
    ...
    def forward(self, images, labels=None):
        ...
        for b in range(images.shape[0]):
            ...
            f_0 = output[pre_0]
            grad_f_0 = torch.autograd.grad(
                f_0, image, retain_graph=False, create_graph=False
            )[0]
            ...
            for i in range(self.steps):
                ...
                for k in range(num_classes):
                    ...
                    f_k = output[k]
                    grad_f_k = torch.autograd.grad(
                        f_k, image, retain_graph=True, create_graph=True
                    )[0]
                    ...
                    if r is None:
                        r = (
                            torch.abs(f_prime) / (torch.norm(grad_f_prime) ** 2)
                        ) * grad_f_prime
                        min_value = value
                    else:
                        if min_value > value:
