...
def get_criterion(
    criterion_type: str, reduction: str = "mean", weight: torch.Tensor = None
):
    if criterion_type == "MSE":
        if weight is not None:
            warnings.warn(
                "MSELoss does not support weighted loss, weight ignored", Warning
            )
...
def train_step(
    model, train_loader, criterion, optimizer, epoch, device, log_interval=10
):
    model.train()
    train_loss = 0.0
    correct = 0
...
def test_step(model, test_loader, criterion, device, epoch=0.0, silent=False):
    model.eval()
    test_loss = 0.0
    correct = 0
    total = 0
    ...
    if not silent:
        print(
            "[Test]  Epoch: {}\tLoss: {:.6f}\tAcc: {:.1f}% ({}/{})".format(
                epoch, test_loss, acc, correct, total
            )
        )
    ...
    **kwargs
    ...
    if weighted_loss:
        if not isinstance(trainset.samples[0][1], int):
            print(
                "Labels in trainset is of type: {}. Expected: {}.".format(
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        if test_loader is not None:
            test_loss, test_acc = test_step(
                model, test_loader, criterion_test, device, epoch=epoch
            )
...
class Model(object):
    def __init__(
        ...
        **kwargs
        ...
        if complexity == -1 and channel == 3:
            self.model = zoo.get_net(
                model_arch, input_size, pretrained, num_classes=num_classes
            ).to(device)
        else:
    ...
    def test(self, testset: Dataset = None, silent=False):
        ...
        if testset is None:
            ...
            test_loader = DataLoader(
                testset, self.batch_size, shuffle=False, num_workers=self.num_workers
            )
        ...
        if not silent:
            print(
                "[Test]  Epoch: {}\tLoss: {:.6f}\tAcc: {:.1f}% ({}/{})".format(
                    self.epoch, test_loss, acc, correct, total
                )
            )
