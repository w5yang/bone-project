...
def main():
    ...
    parser.add_argument(
        "--train-subset", type=int, help="Use a subset of train set", default=None
    )
    ...
    complexity = (
        args.complexity
    )  # if a model does not contain differernt complexity variance, this param will
    # have NO effects.
    model = zoo.get_net(
        model_name,
        input_size,
    ...
    optimizer = model_utils.get_optimizer(
        model.parameters(), params["optimizer_choice"], **params
    )
